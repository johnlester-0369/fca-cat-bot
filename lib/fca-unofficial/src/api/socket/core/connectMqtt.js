...
module.exports = function createListenMqtt(deps) {
  ...
  return function listenMqtt(defaultFuncs, api, ctx, globalCallback) {
    ...
    mqttClient.on("close", function () {
      ...
      // Reconnect on unexpected close — mirrors error handler; ctx._ending guard inside scheduleReconnect prevents reconnect on intentional stopListening
      scheduleReconnect();
    });
    ...
    mqttClient.on("disconnect", () => {
      ...
      // Server-initiated disconnects do not fire close; reconnect here so session is not permanently lost
      scheduleReconnect();
    });
  };
};
